...
for n_id in 8 384; do
  for method in hard adaptive released; do
    for seed in 0 1 2; do
      ...
      output="${BASE}/identity_scaling20k_head_target_v1/${label}"
      ...
      CUDA_VISIBLE_DEVICES="${slot}" "${ROOT}/scripts/run_py310.sh" \
        ...
        --steps 20000 --training-region head_roi \
...
if (( ${#failures[@]} > 0 )); then
  printf '%s\n' "${failures[@]}" >"${BASE}/identity_scaling20k_head_target_v1/failed_runs.txt"
  echo "Head-target failures=${#failures[@]} labels=${failures[*]}" >&2
  exit 1
fi
rm -f "${BASE}/identity_scaling20k_head_target_v1/failed_runs.txt"
echo "Completed head-target identity scaling controls"
