...
OUTPUT_ROOT="${1:-${ROOT}/artifacts/runs/multicase_mean_extent_density4000}"
...
radii=(0 0 -1 -1)
caps=(3 30 3 30)
density_names=(d025 d400)
density_values=(0.25 4.0)
...
for density_index in "${!density_names[@]}"; do
  density_name="${density_names[$density_index]}"
  density_value="${density_values[$density_index]}"
  for case_index in "${!subjects[@]}"; do
    ...
    for condition_index in "${!condition_names[@]}"; do
      ...
      output="${OUTPUT_ROOT}/${density_name}/${subject}/${name}"
      mkdir -p "${output}"
      CUDA_VISIBLE_DEVICES="${slot}" "${ROOT}/scripts/run_py310.sh" \
        ...
        --density-multiplier "${density_value}" \
        --steps 4000 \
        --seed 0 \
        --output "${output}" \
...
if (( failures > 0 )); then
  echo "mean-extent-density factorial failed_runs=${failures}" >&2
  exit 1
fi

echo "Completed multi-case mean-extent-density factorial: ${OUTPUT_ROOT}"
