...
OUTPUT_ROOT="${OUTPUT_ROOT:-$ROOT/artifacts/runs/multicase_geometry_seed0}"
STEPS="${STEPS:-600}"
HEIGHT="${HEIGHT:-128}"
...
if (( failures > 0 )); then
  echo "multicase geometry failed_runs=$failures" >&2
  exit 1
fi
echo "multicase geometry completed runs=20"
